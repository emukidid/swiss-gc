...
// utility functions
int load( const char * name, void ** data )
{
    size_t size = 0;
    *data = NULL;
...
void save(const char * name, const void * data, int size)
{
...
int main (int argc, char * const argv[]) 
{
    ...
    if (argc == 4) {
        ...
        const char* last_fwdslash = strrchr( argv[1], '/' );
        const char* last_backslash = strrchr( argv[1], '\\' );
        // Windows allows mixed forward and back slash characters (whyyyyy)
        if ( last_fwdslash && last_backslash )
            name = last_fwdslash > last_backslash ? last_fwdslash+1 : last_backslash+1;
        else if ( last_fwdslash )
            name = last_fwdslash+1;
    ...
    strncpy((char*)(gci_data + 0x08), name, 31); // copy name
    ...
    set_u16be(gci_data + 0x38, data_blocks);     // number of 8KiB blocks in file
    ...
    strncpy((char*)(gci_data + 0x160), name, 31);
