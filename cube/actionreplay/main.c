...
extern unsigned char AUTOEXEC_DOL[];
extern unsigned long AUTOEXEC_DOL_size;
...
{
	int i;
	DOLHEADER *hdr = (DOLHEADER *) AUTOEXEC_DOL;

	// Inspect text sections to see if what we found lies in here
	for (i = 0; i < MAXTEXTSECTION; i++) {
		if (hdr->textAddress[i] && hdr->textLength[i]) {
			_memcpy((void*)hdr->textAddress[i], ((unsigned char*)AUTOEXEC_DOL) + hdr->textOffset[i], hdr->textLength[i]);
		}
	}
	...
	for (i = 0; i < MAXDATASECTION; i++) {
		if (hdr->dataAddress[i] && hdr->dataLength[i]) {
			_memcpy((void*)hdr->dataAddress[i], ((unsigned char*)AUTOEXEC_DOL) + hdr->dataOffset[i], hdr->dataLength[i]);
		}
	}
