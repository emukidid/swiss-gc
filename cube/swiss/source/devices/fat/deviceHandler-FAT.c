	...
	if(swissSettings.emulateMemoryCard) {
		if(devices[DEVICE_CUR] != &__device_sd_a && devices[DEVICE_CUR] != &__device_ata_a && devices[DEVICE_CUR] != &__device_ata_c) {
			...
			if(swissSettings.memoryCardA[0]) {
				concatf_path(patchFile.name, devices[DEVICE_CUR]->initial->name, "swiss/saves/%s", swissSettings.memoryCardA);
			} else {
		...
		if(devices[DEVICE_CUR] != &__device_sd_b && devices[DEVICE_CUR] != &__device_ata_b) {
			...
			if(swissSettings.memoryCardB[0]) {
				concatf_path(patchFile.name, devices[DEVICE_CUR]->initial->name, "swiss/saves/%s", swissSettings.memoryCardB);
			} else {
