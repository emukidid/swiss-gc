...
int findCheats(bool silent) {
	...
	sprintf(cheatsFile->name, "%.*s/cheats/%.6s.txt", PATHNAME_MAX-19, devices[DEVICE_CUR]->initial->name, trimmedGameId);
	...
	if(devices[DEVICE_TEMP]->readFile(cheatsFile, &testBuffer, 8) != 8) {
		...
		memset(cheatsFile, 0, sizeof(file_handle));
		sprintf(cheatsFile->name, "%.*s/cheats/%.6s.txt", PATHNAME_MAX-19, slotFile->name, trimmedGameId);
		print_gecko("Looking for cheats file @ [%s]\r\n", cheatsFile->name);
